...
def main(argv):
    ...
    (train_x, train_y), (test_x, test_y), (voc_item, voc_store) = store_data.load_data(args.test_percent, args.test_selection)
    prices = test_x["unit_price"]

    # IDs
    stores = tf.feature_column.categorical_column_with_vocabulary_list("store", voc_store)
    item = tf.feature_column.categorical_column_with_vocabulary_list("item", voc_item)
    ...
    ind = {
        ...
        "item": tf.feature_column.indicator_column(item)
    }
    ...
    # Feature columns
    feature_cols = [
            ind["item"], ind["stores"], ind["department"], ind["promo"], unit_price, sin_time, cos_time, sin_day, cos_day,
        ]
